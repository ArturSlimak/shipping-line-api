...
public class VoyageService {
    ...
    private Voyage mapCreateVoyageRequestToVoyage(CreateVoyageRequest voyageRequest){
        ...
        //check for vessel,port ids in voyage payload;
            Vessel vessel = vesselRepository.findById(voyageRequest.getVesselId())
                    .orElseThrow(() -> new IllegalArgumentException(("Vessel not found")));
            Port arrivalPort =
                    portRepository.findById(voyageRequest.getArrivalPortId())
                            .orElseThrow(() -> new IllegalArgumentException("Arrival port not found "));
            Port departurePort = portRepository.findById(voyageRequest.getDeparturePortId())
                    .orElseThrow(() -> new IllegalArgumentException("departure port not found"));
            //check for departure time is in future
            if(!voyageRequest.getDepartureTime().isAfter(LocalDateTime.now()) ) throw new IllegalArgumentException("Departure date must be in future");
            //check for arrival time is after daparture time
            if(voyageRequest.getArrivalTime().isBefore(voyageRequest.getDepartureTime())) throw new IllegalArgumentException("arrival date must be after departure date");
    ...
    public VoyageService(VoyageRepository voyageRepository, VesselRepository vesselRepository, PortRepository portRepository) {
